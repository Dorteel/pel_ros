...
def create_robot_kg_ssn():
    ...
    for topic in latest_messages.keys():
        if 'sensors' in topic:
            ...
            _, robot_id, _, annotator_type, annotator_name = topic.split('/')[:5]
            annotator = kg_namespace[annotator_name]
            robot_kg.add((annotator, RDF.type, SOSA.Procedure))
            robot_kg.add((annotator, SSN.implementedBy, system))       
    ...
    with lock:
        for topic, (msg, timestamp) in latest_messages.items():
            ...
            if 'sensors' in topic:
                ...
                _, robot_id, _, annotator_type, annotator_name = topic.split('/')[:5]
                result_name = f'res_{annotator_name}_{timestamp}'
                result = kg_namespace[result_name] #rdflib.Literal(msg)
                graph.add((result, RDF.type, SOSA.Result))
                observation = kg_namespace[f'obs_{annotator_name}_{timestamp}']
                graph.add((observation, RDF.type, SOSA.Observation))
                graph.add((observation, SOSA.usedProcedure, kg_namespace[annotator_name]))
                graph.add((observation, SOSA.resultTime, rdflib.Literal(timestamp)))
